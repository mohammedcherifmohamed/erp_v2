...
@section('page-subtitle', 'Quiz details')
...
<div class="space-y-6">
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="card lg:col-span-2">
            ...
            <div class="card-body p-0">
                <div class="divide-y">
                    ...
                    @empty
                        <p class="text-center text-gray-500 py-8">No questions in this quiz</p>
                    @endforelse
                </div>
            </div>
                ...
                <div class="table-container">
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            @empty
                                <tr><td colspan="6" class="text-center text-gray-500 py-8">No submissions yet</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
        ...
        <div class="card">
            ...
            <div class="card-body space-y-4">
                ...
                <div>
                    <p class="text-sm text-gray-500">Passing Points</p>
                    <p class="font-medium">{{ $quiz->passing_points }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-500">Time Limit</p>
                    <p class="font-medium">{{ $quiz->time_limit ? $quiz->time_limit . ' min' : 'No limit' }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-500">Published</p>
                    <span class="badge-{{ $quiz->is_published ? 'success' : 'warning' }}">{{ $quiz->is_published ? 'Published' : 'Draft' }}</span>
                </div>
                <div>
                    <p class="text-sm text-gray-500">Available</p>
                    <p class="text-sm">{{ $quiz->available_from ? $quiz->available_from->format('M d, Y H:i') : 'Always' }} @if($quiz->available_until) - {{ $quiz->available_until->format('M d, Y H:i') }} @endif</p>
                </div>
            </div>
        </div>
    </div>
    <div class="flex gap-3">
        ...
        <a href="{{ route('teacher.quizzes.correct', $quiz) }}" class="btn-success">Correct Submissions</a>
        <a href="{{ route('teacher.quizzes.index') }}" class="btn-outline">Back to Quizzes</a>
    </div>
</div>
@endsection
