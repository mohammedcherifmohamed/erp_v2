...
<div class="space-y-4">
    ...
    <div class="grid grid-cols-7 gap-2">
        ...
        @foreach($days as $index => $day)
            <div class="border rounded-lg {{ $colors[$index] }} min-h-[400px]">
                ...
                <div class="p-1 space-y-1">
                    @forelse(($weeklySchedules[$day] ?? collect()) as $schedule)
                        <div class="bg-white rounded p-1.5 shadow-sm border text-xs space-y-0.5 cursor-default hover:shadow-md transition-shadow">
                            <p class="font-medium text-gray-900 truncate">{{ $schedule->course->name }}</p>
                            <p class="text-gray-500 truncate">{{ $schedule->course->class->name ?? '-' }}</p>
                            ...
                            @if($schedule->classroom)
                                <p class="text-gray-400">{{ $schedule->classroom }}</p>
                            @endif
                        </div>
                    @empty
