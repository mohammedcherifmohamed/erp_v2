...
@section('page-subtitle', 'Emploi du temps de ' . $student->full_name)
        ...
        @foreach($dayKeys as $index => $dayKey)
            <div class="border rounded-lg {{ $colors[$index] }} min-h-[300px]">
                <div class="p-2 border-b border-inherit bg-white/50 rounded-t-lg">
                    <h3 class="text-xs font-semibold text-gray-700 uppercase">{{ $days[$index] }}</h3>
                </div>
                <div class="p-1 space-y-1">
                    @forelse(($week[$dayKey] ?? collect()) as $schedule)
                        <div class="bg-white rounded p-2 shadow-sm border text-xs space-y-1 cursor-default hover:shadow-md transition-shadow">
                            ...
                            @if($schedule->classroom)
                                <p class="text-gray-400">Salle : {{ $schedule->classroom }}</p>
                            @endif
                        </div>
                    @empty
